    ...
    try:
        additional = np.array(
            [
                ...
                np.arctan2(
                    env.target_position[1] - env.lander_position[1], env.target_position[0] - env.lander_position[0]
                ),
        ...
        raise ValueError(
            "Target position must be defined in environment to use target landing observation. "
...
def get_observation_function(name: str):
    ...
    """
    mapping = {"default": (default_observation, 8), "landing_zone": (target_landing_observation, 13)}
